...
include_graphics(here("analysis/images/watershed_map.jpg"))
```
...
half_hr_periods |>
  ...
  kable(booktabs = T,
        linesep = "",
        format.args = list(big.mark = ","),
        caption = "Number of half hour periods each year between February and mid-June, how many when the SONAR was operational, how many were reviewed and what percentage it was operational and reviewed, 2018 - 2022.") |> 
  kable_styling(latex_options = c("hold_position"))
```
...
```{r fl-hist, fig.height = 4, fig.cap = "Histogram of forklengths, colored by species. Dashed lines are the overlap range between steelhead and other species."}
spp_fl %>%
...
sonar_pred_spp |> 
  ...
  ungroup() |> 
  pivot_wider(names_from = c(direction,
                             gr_67),
              values_from = c(n_fish:tot_sthd),
              names_vary = "slowest") |>
  select(-c(tot_sthd_downstream_FALSE,
            tot_sthd_upstream_FALSE)) |> 
  kable(col.names = c("Year",
                      rep(c(rep(c("n Fish",
                            "Likely Sthd",
                            "n Sthd"),
        ...
        booktabs = T,
        linesep = "",
        caption = "Number of fish observed moving downstream or upstream, less than or greater than 67 cm, and whether they were likely (> 50% probability) to be a steelhead. Totals within each movement and size group are shown (n Sthd) as well as totals for each year by direction (Total Sthd).") |> 
...
sonar_pred_spp |> 
  ...
  kable(col.names = c("Direction",
        ...
        booktabs = T,
        linesep = "",
        caption = "Number of fish observed moving downstream or upstream (n Fish), less than or greater than 67 cm, and whether they were likely (> 50% probability) to be a steelhead according to the species composition GAM. Totals within each movement and size group are shown (n Sthd) as well as totals for each year by direction (Total Sthd). Note that for fish greater than 67 cm, we assumed they were all steelhead, regardless of the GAM predictions.") |>
  kable_styling(latex_options = c("scale_down",
                                  "hold_position")) |> 
  collapse_rows(c(1)) |> 
  column_spec(c(9),
...
yr_est |> 
  ...
  kable(col.names = c("Year",
        ...
        booktabs = T,
        linesep = "",
        caption = "Annual estimates of winter steelhead in the early (upstream - downstream) and late (upstream only) periods as well as total upstream escapement to the Dungeness River between Feb 1 and June 15.") |> 
  kable_styling(latex_options = c("scale_down",
                                  "hold_position")) |> 
...
comp_est |> 
  ...
  kable(col.names = c("Year",
        ...
        booktabs = T,
        linesep = "",
        caption = "Annual estimates of upstream escapement of winter steelhead to the Dungeness River between Feb 1 and June 15, including counts of observed fish (from SONAR) and redd-based estimate of spawners.") |> 
  kable_styling(latex_options = c("hold_position"))
```

```{r redd-fig, fig.cap = "Annual estimates of upstream escapement of winter steelhead to the Dungeness River between Feb 1 and June 15, including counts of observed fish (from SONAR) and redd-based estimate of spawners."}
pd = 0.2
comp_est |>
  ggplot(aes(x = year)) +
